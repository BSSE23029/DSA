...
template <typename T> void MyArray<T>::countSort(T *arr, int n, T exp) {
...
template <typename T> void MyArray<T>::radix_sort(T *array, size_t n) {
  T *queue0 = new T[n]; // separate queue for 0-bits
  T *queue1 = new T[n]; // separate queue for 1-bits
  size_t count0 = 0; // count of elements in queue0
  size_t count1 = 0; // count of elements in queue1
  ...
  for (T bit = 1; bit; bit <<= 1) {
    ...
    for (size_t k = 0; k < n; ++k) {
      if (array[k] & bit) {
        queue1[count1++] = array[k];
      } else {
        queue0[count0++] = array[k];
      }
    }
    ...
    for (size_t j = count0; j < n; ++j, ++k) {
      array[j] = queue1[k - count0];
    }
  }
...
template <typename T> void MyArray<T>::radixSort1D(T *&array) {

  radix_sort(array, d0);
}

template <typename T> void MyArray<T>::radixSort2D() {
...
template <typename T> void MyArray<T>::radixSort3D() {
  // for (int i; i < d1; i++) {
  //   for (int j; j < d2; j++) {
  //     radixSort1D(arr3D->getRow3D(i, j));
  //   }
  // }
...
template <typename T> void MyArray<T>::countSort1D(T *&array) {
  countSort(array, d0);
}

template <typename T> void MyArray<T>::countSort2D() {
  T *arr = new T[d0];
...
template <typename T> void MyArray<T>::countSort3D() {
